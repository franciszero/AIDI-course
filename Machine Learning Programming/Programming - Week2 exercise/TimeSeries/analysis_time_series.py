...
class TimeSerious:
    ...
    def job4_calc_surpass_times(self):
        ...
        mpf.plot(d, volume=True, style='yahoo', type='line', savefig=path, title=title)  # type='candle'

    def plot_KDE(self, ax1, dist, l, m, r, x, x_test, y):
        # prepare twin plot ax
        ax2 = ax1.twiny()
        [t.set_color('red') for t in ax1.xaxis.get_ticklabels()]
        [t.set_color('skyblue') for t in ax2.xaxis.get_ticklabels()]
        # histogram of residuals
        ax2 = plt.hist(y, 80, color='skyblue', histtype='stepfilled', orientation='horizontal', alpha=0.8)
        # KDE
        ax1.plot(dist, x_test, '-.', color='r', linewidth=.8, alpha=1)
        ax1.axis(xmin=0, xmax=max(dist) * 1.1)
        # plot anomalies threshold in residuals
        ax1.plot(x, [l] * len(x), color='gray', linewidth=.2, alpha=.8)
        ax1.plot(x, [r] * len(x), color='gray', linewidth=.2, alpha=.8)
        ax1.plot(x, [m] * len(x), color='red', linewidth=.9, alpha=1)
